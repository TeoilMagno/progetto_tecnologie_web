...
function AppLayout() {
  ...
  return (
    <div className="flex h-[100dvh] w-full bg-slate-950 text-white font-sans overflow-hidden flex-col relative">
      
      {/* --- BACKGROUND GLOBALE FISSO (Visibile su tutte le sezioni tranne Map/Quiz se necessario) --- */}
      {!hideGlobalUI && (
        <div className="fixed inset-0 z-0 pointer-events-none">
          ...
          <div 
            ...
            style={{ 
              backgroundImage: 'linear-gradient(to bottom, rgba(15, 23, 42, 0.45) 0%, rgba(15, 23, 42, 0.85) 50%, rgba(15, 23, 42, 0.98) 100%)'
            }}
          />
        </div>
      )}
      ...
      {/* --- HEADER MUSEO GLOBALE (Nascosto su /map e /quiz) --- */}
      {!hideGlobalUI && (
        <header className="bg-slate-900/80 backdrop-blur-lg border-b border-slate-800 px-4 py-4 shrink-0 z-50 w-full relative">
      ...
      <div className="flex-1 flex flex-col relative w-full h-full overflow-hidden z-10">
        ...
        {/* --- MOBILE BOTTOM BAR (Nascosta su /map e /quiz) --- */}
        {!hideGlobalUI && (
          <nav className="bg-slate-900/80 backdrop-blur-lg border-t border-slate-800 pb-safe pt-2 shrink-0 z-50 w-full relative">
